...
- **`winfm-conpty-limits-rnd` is RESOLVED**, with an answer rather than by being forgotten. It asked whether a native Windows facility could close these gaps, and both candidates are rejected on structural grounds rather than on a reading recorded here: **console control events** (`GenerateConsoleCtrlEvent`) address a console process group, which is a creation-time attribute this process never holds, and adopting one would have broken `interrupt` — the one verb that already worked; and **job objects**, whose kill is a force-kill, so they cannot give a graceful stop at all. Those are properties of the Win32 APIs rather than measurements, and the investigation that probed them wrote its report outside this tracked tree: no probe output for either candidate is in [`runtime-backends.md`](verification/runtime-backends.md), and none should be looked for there. Neither is to be revisited. What shipped instead is not a native facility but an inline one: OSC 133 semantic prompt marks read off the pty byte stream the daemon already owns, which is what the section above describes.
