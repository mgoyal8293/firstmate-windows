...
The two hooks that carry the marks are exported, so a shell nested inside the session continues the chain rather than silencing it.
That is a requirement, not an optimisation: a task session runs its agent inside the worktree provider's subshell, which lives for the whole task, so marks that stopped at the shell firstmate armed would leave the last mark saying "a command is running" forever and no stop could ever be proven.
...
- **Foreground scoping depends on the session shell being bash, and being able to speak.** The prompt marks that recover tmux's foreground scoping need `PS0`, so bash 4.4 or newer; every Git for Windows release whose notes mention a bash version ships 4.4 or later, so this is a rare path rather than a common one. A session whose shell is not bash, or whose rc files overwrite `PS0` and `PROMPT_COMMAND`, emits no mark and falls back to the console list plus the screen — where a harness left running in the *background* of an idle session can still classify `alive`. The failure direction is the safe one, recovery suppressed rather than a duplicate agent launched, and `exit` then refuses rather than claiming a stop it cannot prove.
- The worktree provider opens its subshell with `$SHELL`, falling back to `%COMSPEC%`. cmd.exe announces no OSC 0 title, so worktree discovery cannot see the session leave the project, and it can emit no prompt mark. The rcfile therefore repairs an absent or unresolvable `SHELL`; a session shell that never loads the rcfile still has both problems.
