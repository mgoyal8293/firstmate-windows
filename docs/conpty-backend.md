...
- **Foreground scoping depends on the session shell being bash, and being able to speak.** The prompt marks that recover tmux's foreground scoping need `PS0`, so bash 4.4 or newer; every Git for Windows release whose notes mention a bash version ships 4.4 or later, so this is a rare path rather than a common one. A session whose shell is not bash, or whose rc files overwrite `PS0` and `PROMPT_COMMAND`, emits no mark and falls back to the console list plus the screen. That fallback is no longer a dead end for `exit`: because the spawn path leaves no worktree-provider process attached to the console, a session sitting at a recognisable shell prompt with only shells attached reads `dead` there too. What the fallback cannot narrow is a background harness under a prompt the screen heuristic does not recognise, which still classifies `alive` — recovery suppressed rather than a duplicate agent launched.
...
- **The pool slot is released by teardown, not by a dying subshell.** Leasing the worktree is what removes the provider subshell, so `fm-teardown` returning it (`treehouse return --force <dir>`) is now the only thing that frees the slot. A crash that never reaches teardown leaves the slot leased — visibly, and attributable, because the holder is recorded as `firstmate-<id>` — and one `treehouse return --force <path>` clears it.
- `treehouse get` opens `$SHELL`, falling back to `%COMSPEC%`. cmd.exe announces no OSC 0 title, so worktree discovery cannot see the session leave the project. The rcfile therefore repairs a `SHELL` with no Windows executable image behind it (the value, or the value plus `.exe`, must begin with the PE magic `MZ`, which is what `treehouse.exe` needs to `CreateProcess` it); a session shell that never loads the rcfile is not repaired.
