...
- **The control plane now admits conpty, but no verb has been re-measured on a real host.** Both gates in `bin/fm-control-lib.sh` name this backend: `fm_control_backend_state_verified` admits it to `exit` and `relaunch` because `fm_backend_conpty_agent_state` is a real recovery-grade classifier (dispatched from `bin/fm-backend.sh`, validating pid identity by name and start time), and the key matrix `fm_control_backend_supports_key` admits it to `interrupt` because `fm_backend_conpty_normalize_key` handles Escape, Enter, Ctrl+C and Ctrl+U. That admission is proven only against a stubbed session provider, by `tests/fm-control.test.sh`, which asserts the trusted list against `fm_backend_agent_state`'s own answers rather than restating it. The end-to-end Windows run recorded in [`windows.md`](windows.md) "Run end to end on Windows" predates the admission: at that time `exit` refused with `no recovery-grade agent-state classifier`, and stopping the agent fell to `bin/fm-teardown.sh`'s reaper, which force-kills the leaked worktree processes. Whether a graceful stop actually completes on a real ConPTY session is therefore still unverified, tracked as `winfm-conpty-graceful-stop` (queued in firstmate-windows), with `winfm-conpty-limits-rnd` separately investigating whether native Windows facilities can close the remaining gaps.
