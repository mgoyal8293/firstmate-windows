...
- **The control plane's state-verified allowlist does not include conpty.** `exit` and `relaunch` are gated on `fm_control_backend_state_verified` in `bin/fm-control-lib.sh`, which still lists only `tmux` and `herdr`, so `bin/fm-control.sh <id> exit` refuses rather than report a stop it cannot prove. This adapter does implement a recovery-grade agent-state classifier - `fm_backend_conpty_agent_state`, dispatched from `bin/fm-backend.sh` and validating pid identity by name and start time - so the boundary is that allowlist, whose wording predates this backend, and not a missing classifier. `interrupt` is unaffected, because it is non-destructive and its endpoint-existence postcondition still holds. Measured end to end on a real session: the refusal names the backend, and stopping the agent then falls to `bin/fm-teardown.sh`'s reaper, which force-kills the leaked worktree processes. There is no graceful agent shutdown on this backend yet.
