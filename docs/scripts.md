...
| `fm-bearings-snapshot.sh` | Project the fleet snapshot to the compact TOON bearings view; live PR discovery only under `--include-prs` |
...
| `fm-test-run.sh`         | Behavior-test runner: selection, portable and Windows lanes, proven-isolated `--jobs`, coverage guard, timing/JSON |
...
| `fm-backend-hometag-lib.sh` | Shared per-installation home-tag derivation for zellij tab titles, cmux workspace titles, and conpty session/pipe names |
...
| `backends/conpty.sh`     | Experimental Windows ConPTY session-provider adapter, where tmux does not exist       |
| `backends/conpty/`       | The ConPTY session daemon and its stateless client; firstmate ships these because a Windows pseudoconsole dies with its creating process |
...
| `fm-inactive-reconcile.sh` | Reconcile long-inactive direct crewmate terminal outcomes, making no forge call of its own beyond the one budgeted read inside `fm-crew-state.sh` |
...
| `fm-crew-run-verdict-lib.sh` | Single owner of the crew run-verdict model: candidate selection, code-identity binding, and the CI and forge evidence gates |
