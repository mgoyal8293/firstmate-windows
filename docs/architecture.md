...
The one exception, in normal mode only, is a busy pane whose crew's own last status line declares a bounded external wait or a captain hold: a declaration plus a live harness is the least wedge-like state observable, so that pane takes the declared-pause recheck cadence below instead of the wedge timer, and returns to the wedge timer on the first poll after the declaration stops standing.
While that declaration stands the wedge escalation counter and the `FM_WEDGE_DEMAND_INSPECT_COUNT` `demand-deep-inspection` marker do not accumulate, so a crew that declares a wait and then stops updating its status is covered only by the `FM_PAUSE_RESURFACE_SECS` recheck.
Away mode is outside that exception, because the supervise-daemon owns triage there and drops a declared pause marker on a busy verdict rather than re-surfacing it, so a busy pane past the bound still takes the ordinary wedge escalation and the away-mode wake stream is unchanged.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: an actively running no-mistakes step attributed to that crew, or an exact busy verdict from the semantic busy-state contract.
...
A crew that declares `paused:` for a known external wait, or carries a verified `captain-held` transfer, is separately absorbed by the normal-mode watcher and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge, whether its pane sits idle or still renders as busy.
...
That read is bounded by at most a third of the aggregate budget the scan still has left and is skipped when what remains cannot spare it, because an unread merge state is an unconfirmed answer and no path resolves one to `done`, as the forge-read invariant in `bin/fm-crew-state.sh`'s header records.
...
The default path adds no live PR discovery or checks, which exist only behind the bearings `--include-prs` opt-in, so the snapshot's own bounded per-task forge read above stays its only network cost.
...
[`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) owns new-spawn backend selection precedence and authorization, and the `backend=` metadata contract including the missing-field compatibility default.
...
Crewmates never intentionally touch your project clone; [treehouse](https://github.com/kunchenguid/treehouse) pools clean worktrees for tmux, herdr, zellij, cmux, and conpty tasks, while Orca creates its own worktrees for `backend=orca`.
