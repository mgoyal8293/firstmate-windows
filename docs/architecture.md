...
A crew that declares `paused:` for a known external wait is separately absorbed by the normal-mode watcher and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge, whether its pane sits idle or still renders as busy.
