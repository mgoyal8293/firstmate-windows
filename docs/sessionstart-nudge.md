...
`bin/fm-sessionstart-run.sh` exports `FM_SESSION_START_UNDER_HOOK=1` so the digest can positively establish that it is one of these hook invocations rather than infer it, and the clamp applies both under a hook and when that context cannot be established - it is skipped only on a positively established direct terminal run, where nothing kills the process at the registered timeout.
The truncation banner's own remedy is derived from the same ceiling: below it, how far the bound may still be raised; already at it, that raising cannot help and the SessionStart registrations are the knob that moves.
