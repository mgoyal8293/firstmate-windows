...
Every registered run-tier hook timeout sits STRICTLY above the highest of those defaults, so firstmate's own bound is always the one that bites and the harness never preempts the banner.
That nesting is asserted rather than documented: [`../tests/fm-session-start-hook-nesting.test.sh`](../tests/fm-session-start-hook-nesting.test.sh) discovers every tracked JSON hook registration, derives the ceiling by running `fm_session_start_default_budget` over every platform arm rather than quoting a number, and fails when any digest-running session-start hook is registered at or below it - including a newly added harness whose wrapper it does not recognize.
A harness kill is not survivable the way the digest's own bound is: it takes the parent with the child, so there is no `STARTUP TRUNCATED` banner, no named stage, no per-stage attribution and no reconcile list, and the undrained wake queue and missing supervision instructions become silent.
That is why an explicit `FM_SESSION_START_TIMEOUT` above the shortest registered digest-tier hook timeout is clamped just below it instead of being honoured, with the clamp named on the digest: `fm_session_start_hook_ceiling` reads the same registrations to derive that ceiling, so raising the hook timeouts raises what an operator is allowed to ask for.
The clamp is keyed on whether a KILL DEADLINE actually binds this process rather than on whether this is a hook, because the Pi run tier is a hook that arms no deadline of any kind and clamping it against registrations for harnesses that are not running would take bound away for a kill that cannot happen.
Each transport therefore declares its own deadline status at its own spawn site through `FM_SESSION_START_HOOK_DEADLINE`: `.pi/extensions/fm-primary-turnend-guard.ts` declares `none`, and `bin/fm-sessionstart-run.sh` defaults an undeclared transport to `binds` as well as exporting `FM_SESSION_START_UNDER_HOOK=1` so the digest can positively establish hook invocation rather than infer it.
So the clamp applies when a deadline binds and when it cannot be established either way, and is skipped only where in-repo evidence positively proves none binds - a declaring transport, or a positively established direct terminal run; [`../bin/fm-session-start-bound-lib.sh`](../bin/fm-session-start-bound-lib.sh) owns that resolution.
The truncation banner's own remedy is derived from whatever cap is actually in force on that run: below it, how far the bound may still be raised; already at it, that raising cannot help and the SessionStart registrations are the knob that moves; and where no cap applies at all it asks only for a higher bound and the slow stage, never quoting a ceiling that tier does not enforce.
...
| --- | --- | --- | --- |
| Claude | Run | `.claude/settings.json` registers one unmatched `SessionStart` hook, invoked through `CLAUDE_PROJECT_DIR` with a 360s timeout; the wrapper reads `source` from the hook payload. | Native stdout context injection is supported. |
| Codex exec | Run | `.codex/hooks.json` anchors to the hook process working directory, verifies a Firstmate-shaped hook-bearing root, and pipes the hook payload into the wrapper with a 360s timeout. | Native stdout context injection is supported under `codex exec`. |
