...
The margin there is 22 s against 1 s elsewhere, derived from the worst non-thrashing point of a clean contention sweep on the box: 21608 ms modelled parent-side cost at 16 fork-heavy competitors under real concurrent load, ceiling to whole seconds.
That moves the derived ceiling from 356 s to 338 s, and it replaces a 4 s value the same sweep showed was too small - exceeded under every contended condition measured.
[`verification/session-start-fork-profile.md`](verification/session-start-fork-profile.md) records the sweep, the earlier figures it supersedes, and what it does not claim: total box load rather than competitor count dominates, so 22 s covers the worst measured non-thrashing condition and is not a proof of sufficiency at unbounded load.
[`verification/session-start-fork-profile.md`](verification/session-start-fork-profile.md) records how each number was taken, the open contradiction between 4 s and the contended measurement, the higher-contention re-measurement that will set the final value, and - in its own section - which facts are Windows-measured and which are not yet.
