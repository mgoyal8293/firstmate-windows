...
The margin there is 22 s against 1 s elsewhere, and it is derived from a direct timing of the real parent side on the box: a clamped run whose bounded child was allowed 6 s took 19.1 s in total with its banner intact, so the parent spent 13.1 s outside its own bound at zero contention.
22 s is about 1.7x that.
That moves the derived ceiling from 356 s to 338 s, and it replaces a 4 s value the same box showed was too small - already exceeded before any competitor existed.
[`verification/session-start-fork-profile.md`](verification/session-start-fork-profile.md) records how each number was taken, the modelled figures it supersedes and why they understate the real cost roughly tenfold, which facts are Windows-measured and which are not yet, and what the number does not claim: 13.1 s is one sample at one contention level, and the contended parent side has not been timed directly at all.
