...
Under MSYS that measured 22-subprocess pattern costs 2728 ms at the worst per-fork cost measured under contention on the box, so the margin there is 3 s against 1 s elsewhere, and [`verification/session-start-fork-profile.md`](verification/session-start-fork-profile.md) records how both numbers were taken and what they do not claim.
