...
The margin there is 4 s against 1 s elsewhere, and 4 s is NOT established as sufficient - nor as insufficient, since the per-creation cost that would decide it is currently unmeasured on MSYS.
The figures that once appeared to settle it were taken with orphaned competitor processes still running and have been retracted; a clean contention curve is outstanding.
