...
The clamp is SCOPED to the paths the ceiling governs, because only a harness hook is killed at the hook timeout: it applies when the run is positively under a hook, and also when hook context cannot be established, and is skipped only on a positively established direct terminal invocation - where the operator's own rerun, the remedy this banner prescribes, gets the time they asked for.
...
Uncertainty falls to the clamp on purpose: a wrong "under a hook" costs bound the operator can recover by raising the registrations, while a wrong "not under a hook" costs the whole banner, so the two errors are not interchangeable and the unprovable negative never drives the unsafe branch.
...
The gap between the bound and the shortest registration is per platform too, and it is what pays for the parent's own time: the harness kills the PARENT, which spends about 10 subprocesses on its pre-fork prologue and about 12 more printing the banner after the kill, none of which the bound covers.
The path that has to be covered is the CLAMPED one, since the default 300 s bound never reaches the ceiling and only a clamped bound ever equals it, and that path additionally derives the cap before forking the child.
Under MSYS that pattern costs between 3224 ms and 3968 ms at the worst per-fork cost measured under contention on the box, depending on which count of it you take, so the margin there is 4 s against 1 s elsewhere.
[`verification/session-start-fork-profile.md`](verification/session-start-fork-profile.md) records how both numbers were taken, why 4 s holds across every count on record, that the current tree measures the pessimistic end of that range with 32 ms of slack, and - in its own section - which facts are Windows-measured and which are not yet.
