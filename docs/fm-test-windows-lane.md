...
- On MSYS the session lock is owned by a per-session token rather than by process ancestry (see "How the session lock is owned"), and this suite's fixture strips every harness marker on purpose - so the `LOCK` stage legitimately prints its read-only banner instead of an `acquired`/`held` line. The body pattern covered only the POSIX shapes. That is product behaviour, not a fixture defect, so the pattern now covers the read-only shape too.
