...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over; inspection-only, never an automatic interrupt or restart; a busy pane whose crew's own last status declares a bounded external wait or captain hold takes the FM_PAUSE_RESURFACE_SECS recheck instead
FM_PAUSE_RESURFACE_SECS=3600       # seconds before a declared external wait re-surfaces for a recheck in the watcher or away-mode daemon, whether its pane sits idle or still renders as busy
