...
`tmux` is the verified reference backend (see [`docs/tmux-backend.md`](tmux-backend.md)); `herdr`, `zellij`, `orca`, `cmux`, and `conpty` are experimental spawn backends (see [`docs/herdr-backend.md`](herdr-backend.md), [`docs/zellij-backend.md`](zellij-backend.md), [`docs/orca-backend.md`](orca-backend.md), [`docs/cmux-backend.md`](cmux-backend.md), and [`docs/conpty-backend.md`](conpty-backend.md)).
`conpty` is the Windows session provider, where tmux does not exist; unlike every other adapter it has no third-party multiplexer behind it, because a Windows pseudoconsole dies with its creating process, so firstmate ships the session daemon itself in `bin/backends/conpty/`.
Treehouse remains the worktree provider for tmux, herdr, zellij, cmux, and conpty, since those are session providers only; Orca provides both the task worktree and terminal endpoint.
...
Zellij, Orca, and conpty are never auto-detected; select them by putting the name in a local `config/backend` file, by exporting `FM_BACKEND=<name>`, or by telling the first mate in chat.
conpty is never auto-detected because there is no ambient session to detect: each task's daemon is its own container.
Any value other than `tmux`, `herdr`, `zellij`, `orca`, `cmux`, or `conpty` is rejected until another adapter is implemented and verified.
`fm-spawn.sh` accepts `tmux`, `herdr`, `zellij`, `orca`, `cmux`, and `conpty` for ship and scout tasks; `backend=orca`, `backend=cmux`, and `backend=conpty` all still refuse `--secondmate` until secondmate launch semantics are designed for each.
...
A conpty task additionally records `conpty_session=`, which is the same value as `window=` because a conpty endpoint is a single home-scoped session id rather than a composite; teardown refuses any record where the two disagree.
...
The Kimi installer requires an existing regular non-symlink `~/.kimi-code/config.toml` and the tool requirements owned by [`docs/turnend-guard.md`](turnend-guard.md#compatibility-limits); it validates but never serializes the captain's TOML and refuses before writing when the config is missing, malformed, or surprising or when a tool requirement is unavailable.
...
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux`, `herdr`, `zellij`, `orca`, or `cmux`), `jq` for the JSON-emitting experimental adapters (`herdr`, `zellij`, `cmux`) whose spawn and liveness paths parse the backend's JSON output, and the `treehouse` worktree provider for every session-provider-only backend (`tmux`, `herdr`, `zellij`, `cmux`, `conpty`).
conpty has no session-provider CLI to install and needs no `jq`, so its `PATH` delta is `treehouse` alone; its daemon ships with firstmate and runs on the universal `node`.
conpty's remaining dependency is not a `PATH` command at all but an npm install inside the backend's own directory, so `fm_backend_required_dependency` in `bin/fm-backend.sh` owns it as a sibling hook and bootstrap probes it with the backend's own spawn-time preflight rather than a presence check.
...
A `backend=conpty` home whose daemon dependencies are absent or unloadable reports `MISSING: conpty-backend-deps` with the npm install command for the backend directory, so a home that cannot spawn is never reported healthy; [`conpty-backend.md`](conpty-backend.md) owns that setup detail.
...
FM_PROC_ROOT_OVERRIDE=   # alternate /proc root, owned by fm-proc-lib.sh's fm_proc_root and also read by fm-teardown.sh and fm-cursor-lib.sh, mainly for tests
...
FM_SESSION_START_TIMEOUT=        # seconds bounding the whole session-start digest; unset means the per-platform default, raised under MSYS/MINGW/Cygwin, and an explicit value is clamped under the shortest registered session-start hook timeout - bin/fm-session-start-bound-lib.sh owns both numbers and the clamp
...
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned for this branch's newest run when axi status answers for another branch
FM_CREW_STATE_FORGE_TIMEOUT=10  # seconds allowed for the one forge read that confirms a merged or closed PR before fm-crew-state.sh will claim one; this default serves the interactive single-task read, and all three callers with a fleet to get through narrow it (3 in fm-fleet-snapshot.sh, 3 in fm-classify-lib.sh's crew_absorb_class, a share of what remains in fm-inactive-reconcile.sh)
FM_CREW_STATE_NO_FORGE=         # truthy skips that forge read; a skipped read is an unconfirmed answer, so the affected crew reports a conservative word (working while it is demonstrably still monitoring, else unknown) rather than done - it does not report done with a weaker reason
...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over; inspection-only, never an automatic interrupt or restart; in normal mode a busy pane whose crew's own last status declares a bounded external wait or captain hold takes the FM_PAUSE_RESURFACE_SECS recheck instead
FM_PAUSE_RESURFACE_SECS=3600       # seconds before a declared external wait or verified captain-held transfer re-surfaces for a recheck; the normal-mode watcher applies it whether its pane sits idle or still renders as busy, the away-mode daemon while idle only
...
FM_PENDING_REPLY_UNVERIFIABLE_SENDER_SECS=900   # seconds a pending-reply record whose stored sender identity predates the format tag, on a platform whose ps cannot re-read that format, may defer on bare pid liveness before the recovery delivery is called unknown
# sub-supervisor (bin/fm-supervise-daemon.sh); presence-gated via /afk
FM_SUPERVISOR_BACKEND=             # optional supervisor pane backend override; tmux/herdr only, otherwise detects $TMUX_PANE then HERDR_ENV/HERDR_PANE_ID before falling back to the home's own resolved backend (fm_backend_name; the literal tmux only when bin/fm-backend.sh is not loaded)
