...
conpty has no session-provider CLI to install and needs no `jq`, so its delta is `treehouse` alone; its daemon ships with firstmate and runs on the universal `node`.
