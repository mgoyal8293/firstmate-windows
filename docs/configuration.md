...
FM_CREW_STATE_FORGE_TIMEOUT=10  # seconds allowed for the one forge read that confirms a merged or closed PR before fm-crew-state.sh will claim one; this default serves the interactive single-task read, and callers with a fleet to get through narrow it (3 in fm-fleet-snapshot.sh, a share of what remains in fm-inactive-reconcile.sh)
