...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then tmux; tmux is the verified reference backend (docs/tmux-backend.md), while herdr, zellij, orca, cmux, and conpty are experimental spawn backends (docs/herdr-backend.md, docs/zellij-backend.md, docs/orca-backend.md, docs/cmux-backend.md, docs/conpty-backend.md) - herdr and cmux can also be selected by runtime auto-detection, zellij, orca, and conpty never are (always explicit), conpty is the Windows session provider where tmux does not exist, and codex-app is not accepted; see docs/codex-app-backend.md; inherited by secondmate homes under the primary-authoritative contract in secondmate-provisioning
...
Unknown means the evidence does not settle it, which includes a run that terminated with neither a forge-confirmed landing nor the CI evidence to call it done; that is a run needing a ruling, not a gate anyone can respond to, and the detail line names the ci word and the forge's answer that led there.
A forge-confirmed merge or close does settle done on its own, because it proves the work landed.
