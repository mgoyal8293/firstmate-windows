...
test_matrix_herdr_halfblock_rule_bounds_bare_wrap() {
  ...
  local screen plain out
  # The rule and arrow glyphs are written as LITERAL UTF-8 rather than \uXXXX
  # escapes: bash converts \u through the ambient locale's charset, so under a
  # LANG-less C locale - Git Bash's default - `printf '\u2580'` emits the six
  # characters of the escape instead of the glyph, and this case then compared
  # something that is not a rule row at all against a lib whose patterns are
  # literal bytes. Literal bytes on both sides are locale-independent, which is
