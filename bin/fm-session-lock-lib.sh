...
# Session-token ownership, for the platforms the walk below cannot answer.
# Sourced here so every consumer of this library keeps reaching it unchanged.
# shellcheck source=bin/fm-session-token-lib.sh
. "$(dirname -- "${BASH_SOURCE[0]}")/fm-session-token-lib.sh"
