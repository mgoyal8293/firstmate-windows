...
# Ctrl+U (bin/backends/orca.sh's fm_backend_orca_send_key). conpty normalizes
# all four in fm_backend_conpty_normalize_key.
fm_control_backend_supports_key() {  # <backend> <key>
  local backend=${1-} key=${2-}
  case "$backend" in
    tmux|herdr|zellij|cmux|conpty)
...
# Whether <backend> has a recovery-grade agent-state classifier. tmux, herdr and
# conpty implement fm_backend_agent_state; zellij, orca, and cmux report
