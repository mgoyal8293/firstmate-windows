...
# original answer.
#
# fm_path_gotmpdir_export_line quotes through shell_quote from its caller
...
# The translation is gated on shell_quote being available as well as cygpath,
# because an absent quoter would make `$(shell_quote "$win")` expand to nothing
# and emit a valid-looking `export GOTMPDIR=` line that sends Go's build temp
# somewhere fm-teardown.sh will never clean.
# The untranslated POSIX literal is the right degraded answer: it is exactly what
# every non-Windows host emits today and what Windows emitted before the cygpath
# translation existed, so the failure mode is "no translation" rather than an
# export pointing nowhere.
fm_path_gotmpdir_export_line() {  # <posix-gotmpdir>
  local dir=$1 win
  if fm_platform_is_windows && command -v cygpath >/dev/null 2>&1 \
    && command -v shell_quote >/dev/null 2>&1; then
