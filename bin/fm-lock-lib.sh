...
# shellcheck source=bin/fm-lock-proc-lib.sh
. "$(dirname -- "${BASH_SOURCE[0]}")/fm-lock-proc-lib.sh"
...
fm_lock_has_live_holder() {
  ...
  # No lsof on Git for Windows; bin/fm-lock-proc-lib.sh owns the /proc answer
  # and why it is confined to that platform.
