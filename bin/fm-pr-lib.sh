...
# The private-file trust binding below asserts [ ! -L ] and creates artifacts of
# its own, and on the Windows runtimes both depend on the symlink mode
# bin/fm-proc-lib.sh normalises on source. Sourced here rather than left to the
# caller so a security owner never depends on another file's load order.
# shellcheck source=bin/fm-proc-lib.sh
. "$(dirname -- "${BASH_SOURCE[0]}")/fm-proc-lib.sh"
# The mode half of that binding, capability-gated; see the file for why.
# shellcheck source=bin/fm-file-mode-lib.sh
. "$(dirname -- "${BASH_SOURCE[0]}")/fm-file-mode-lib.sh"
...
fm_pr_private_file_valid() {
  ...
  fm_pr_file_mode_is "$path" "$mode" || return 1
