...
- `PLATFORM: <proven capability failure> - <remediation>` - a capability firstmate cannot run without was PROVEN missing on this machine, not merely assumed.
  Two forms exist today, both on Windows.
  A symlink-creation failure means no fleet lock can ever be acquired and every lock wait would spin forever, so nothing may be dispatched until the captain enables the printed privilege and starts a new session; report it as "this machine cannot create the links firstmate's locks are built from" rather than by its label.
  A flattened `.claude/skills` means this home loads no project skills at all, silently; run the printed per-repo `core.symlinks` correction and re-check out that path, then confirm the skills resolve.
  Neither is a tool install, so never pass them to `bin/fm-bootstrap.sh install`.
