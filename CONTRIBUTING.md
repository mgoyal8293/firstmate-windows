...
It also fails when that signature is present but the structured pipeline step attestation carried in the same body is missing, unparseable, or does not report `review`, `test`, and `document` each with status `completed`; a quota skip or an agent skip on one of those steps is not compliant.
That attestation is why the workflow below states a minimum no-mistakes version - an older release writes only the signature line.
...
Local no-mistakes Test is intent-targeted and must not re-run every `tests/*.test.sh`; `.github/workflows/` owns the broad behavior suite plus platform-specific compatibility lanes.
...
This fork adds a Windows lint job and a sharded Windows behavior lane in [`.github/workflows/windows-ci.yml`](.github/workflows/windows-ci.yml), kept out of `ci.yml` so an upstream sync has nothing to conflict with.
That lane runs a measured subset of `tests/*.test.sh` rather than the whole inventory; [`docs/fm-test-windows-lane.md`](docs/fm-test-windows-lane.md) owns its membership, its evidence, and the worklist of scripts that still fail there.
